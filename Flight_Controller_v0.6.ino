...
Servo ESC_1;                                                        // Output object at 50Hz to front left ESC 
Servo ESC_2;                                                        // Output object at 50Hz to front right ESC
Servo ESC_3;                                                        // Output object at 50Hz to rear left ESC
Servo ESC_4;                                                        // Output object at 50Hz to rear right ESC
...
volatile uint16_t unSwitchDInShared;
volatile uint16_t unSwitchCInShared;
uint32_t ulRollStart;
uint32_t ulPitchStart;
uint32_t ulThrottleStart;
uint32_t ulYawStart;
uint32_t ulSwitchDStart;
uint32_t ulSwitchCStart;

int SWC;                                                            // Renamed unSwitchCIn value to be used in the FC
int SWD;                                                            // Renamed unSwitchDIn value to be used in the FC
int Rx;                                                             // Renamed unRollIn value to be used in the FC 
int Ry;                                                             // Renamed unPitchIn value to be used in the FC 
int Ly;                                                             // Renamed unThrottleIn value to be used in the FC 
int Lx;                                                             // Renamed unYawIn value to be used in the FC 
int ESCRx;                                                          // Rx contribution to ESC outputs
int ESCRy;                                                          // Ry contribution to ESC outputs
int ESCLy;                                                          // Ly contribution to ESC outputs
int ESCLx;                                                          // Lx contribution to ESC outptus
int ESC1;                                                           // Output into ESC 1 (front left) / constrained to cause a stream of zero when transmitter is powered down
int ESC2;                                                           // Output into ESC 2 (front right) / constrained to cause a stream of zero when transmitter is powered down
int ESC3;                                                           // Output into ESC 3 (rear left) / constrained to cause a stream of zero when transmitter is powered down
int ESC4;                                                           // Output into ESC 4 (rear right) / constrained to cause a stream of zero when transmitter is powered down
int ESCOff;                                                         // Outputs a value of 0 when written to ESCs

int ESCRxRange;                                                     // Roll stick input range, calculated intially as a default estimated value
int ESCRyRange;                                                     // Pitch stick input range, calculated intially as a default estimated value
int ESCLyRange;                                                     // Throttle stick input range, calculated intially as a default estimated value
int ESCLxRange;                                                     // Yaw stick input range, calculated intially as a default estimated value

int ESCRxMidpoint;                                                  // Roll stick input centre point, calculated intially as a default estimated value
int ESCRyMidpoint;                                                  // Pitch stick input centre point, calculated intially as a default estimated value
int ESCLyMidpoint;                                                  // Throttle stick input centre point, , calculated intially as a default estimated value
int ESCLxMidpoint;                                                  // Yaw stick inputcentre point, calculated intially as a default estimated value
...
int unRollInMaxLimit = 1800;                                        // Value used in calibration to trigger starting to detect the maximum Roll value
int unRollInLowLimit = 1200;                                        // Value used in calibration to trigger starting to detect the minimum Roll value
int unPitchInMaxLimit = 1800;                                       // Value used in calibration to trigger starting to detect the maximum Pitch value
int unPitchInLowLimit = 1200;                                       // Value used in calibration to trigger starting to detect the minimum Pitch value
int unThrottleInMaxLimit = 1800;                                    // Value used in calibration to trigger starting to detect the maximum Throttle value
int unThrottleInLowLimit = 1200;                                    // Value used in calibration to trigger starting to detect the minimum Throttle value
int unYawInMaxLimit = 1800;                                         // Value used in calibration to trigger starting to detect the maximum Yaw value
int unYawInLowLimit = 1200;                                         // Value used in calibration to trigger starting to detect the minimum Yaw value
...
if(Kill_Switch_Count > 60) // Has no transmitted data been recieved recently?
{
ESC_1.write(ESCOff); 
ESC_2.write(ESCOff);
ESC_3.write(ESCOff);
...
else if(unSwitchDIn > 1500 && unSwitchCIn <= 1250) // Has a calibration been requested and the motors are off?
{
Calibration();  
}
else if(unSwitchCIn <= 1250) // Has no control type been selected?
{
ESC_1.write(ESCOff); 
ESC_2.write(ESCOff);
ESC_3.write(ESCOff);
...
else if(unSwitchCIn > 1250)  // Has control type been selected as proprtional and all previous statements are false
{
...
void Calibration()
{
  ...
  //Midpoints (as soon as controller enters calbration)
